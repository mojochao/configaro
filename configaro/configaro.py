"""Configaro configaro.configaro module.

This module provides the library implementation.
...
def put(*args, **kwargs):
    ...
    Raises:
        ValueError: if any arg is malformed

    """
    _ensure_initialized()
    data = _config_data()  # NOTE: do not remove the 'data' parameter in function as it is used here
    ...
    for arg in args:
        ...
        if isinstance(config[name], dict):
            raise RuntimeError('cannot update a non-scalar configuration property.')
        config[name] = value

    # If the caller doesn't care about nested properties, the top level property
    # names may be passed in keyword args.
...
def render(prop=None):
    """Render configuration as YAML.

    Args:
        prop (str): optional configuration property to render
    ...
    _ensure_initialized()
    data = get(prop)
    rendered = yaml.dump(data, default_flow_style=False)
    return rendered
...
def _merge(dict1, dict2):
    ...
    Args:
        dict1 (dict): original data
        dict2 (dict): deltas data
    ...
    for k in set(dict1.keys()).union(dict2.keys()):
        if k in dict1 and k in dict2:
            if isinstance(dict1[k], dict) and isinstance(dict2[k], dict):
                yield k, dict(_merge(dict1[k], dict2[k]))
            else:
